...
namespace TheMemorableMoments.UI.Web.Services
{
    public class PagedList<T> : List<T>, IPagedList
    {
        ...
        public PagedList(IEnumerable<T> source, int index, int pageSize)
        {
            ...
            //Check that the index is correct for the source and it's current count.
            int skipCount = ((index - 1) * pageSize < TotalCount ? (index - 1)*pageSize : pageSize);
            AddRange(source.Skip(skipCount).Take(pageSize).ToList());
        }
