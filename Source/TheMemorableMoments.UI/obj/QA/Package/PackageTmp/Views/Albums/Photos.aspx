...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div id="fadeinarea" class="hidden">
		...
		<ul style="float:right;" >
           ...
           <li><a class="youarehere" href="/<%= Model.User.Username %>/albums/photos/<%= Model.Album.AlbumId %>">Photos</a></li>
    ...
    <p style="margin-top:40px;" class="addlink" >
        ...
        <%= Html.Grid(Model.Media).Attributes(new Dictionary<string, object>{{"class", "width100percent results"}})
                .Columns(c => c.For(x => (x.Position.HasValue ? x.Position.GetValueOrDefault().ToString() : string.Empty)))
                .Columns(c => c.For(x =>  Model.GetImageLink(x) + "<input class=\"mediaid\" type=\"hidden\" value=\"" + x.MediaId + "\" />").DoNotEncode())
